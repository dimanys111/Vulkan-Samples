...
void HPPTerrainTessellation::draw()
{
    ...
    if (statistics.query_supported) {
        ...
        statistics.results = get_device()->get_handle().getQueryPoolResult<std::array<uint64_t, 2>>(statistics.query_pool, 0, 1, sizeof(statistics.results), vk::QueryResultFlagBits::e64).value;
    }
...
void HPPTerrainTessellation::prepare_uniform_buffers()
{
    ...
    terrain.tessellation_buffer = std::make_unique<vkb::core::HPPBuffer>(*get_device(), sizeof(terrain.tessellation), vk::BufferUsageFlagBits::eUniformBuffer, VMA_MEMORY_USAGE_CPU_TO_GPU);

    // Skysphere vertex shader uniform buffer
    sky_sphere.transform_buffer = std::make_unique<vkb::core::HPPBuffer>(*get_device(), sizeof(sky_sphere.transform), vk::BufferUsageFlagBits::eUniformBuffer, VMA_MEMORY_USAGE_CPU_TO_GPU);
